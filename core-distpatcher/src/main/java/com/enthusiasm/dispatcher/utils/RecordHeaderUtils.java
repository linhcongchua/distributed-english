package com.enthusiasm.dispatcher.utils;
...
public class RecordHeaderUtils {
    ...
    public static <T> T getHeader(ConsumerRecord<String, byte[]> record, String extraHeaderKey, String key, Class<T> clazz) {
        byte[] keyByteValue = getHeaderBytes(record, extraHeaderKey, key);
        return DeserializerUtils.deserialize(keyByteValue, clazz);
    }

    public static byte[] getHeaderBytes(ConsumerRecord<String, byte[]> record, String extraHeaderKey, String key) {
        Headers headers = record.headers();
        ...
        return SerializerUtils.serializeToJsonBytes(keyValue);
    }
}
